...
df = pd.read_csv("AFC_repID_to_GO_master_sheet__memIDCluster_within_PercentPresence_gte_0.75_updated_obsolete_alt.csv")
df_reduced = df.dropna(subset=["GO_term"])
...
def map_to_go(clade_id):
    ...
    sample1 = pd.read_csv(path+"/"+folder_name+"/files_for_scoary/clade_"+clade_id+"_sample_1/g_AFC_presence_absence__collapse_"+COLLAPSE+"__clade_"+clade_id+"_sample_1.csv", usecols=[0], names=["pfam_id"]).dropna()
    sample2 = pd.read_csv(path+"/"+folder_name+"/files_for_scoary/clade_"+clade_id+"_sample_2/g_AFC_presence_absence__collapse_"+COLLAPSE+"__clade_"+clade_id+"_sample_2.csv", usecols=[0], names=["pfam_id"]).dropna()
    sample3 = pd.read_csv(path+"/"+folder_name+"/files_for_scoary/clade_"+clade_id+"_sample_3/g_AFC_presence_absence__collapse_"+COLLAPSE+"__clade_"+clade_id+"_sample_3.csv", usecols=[0], names=["pfam_id"]).dropna()
    pfams_in_genus = set(sample1["pfam_id"]).union(sample2["pfam_id"], sample3["pfam_id"])

    #filter for only genus with pfams
    clade_df = df_reduced[df_reduced["repID"].isin(pfams_in_genus)]
    # Your empty DataFrame
    output = pd.DataFrame(columns=['Clade_id','GO_term', 'Occurrences', 'GO_category'])
    
    for GO_term in clade_df["GO_term"]:
