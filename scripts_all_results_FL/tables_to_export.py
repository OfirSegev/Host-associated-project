...
df_enriched = pd.read_csv("final_results_FL/final_table_all_tests_enriched_results_FL_"+LABEL+"_collapse_"+COLLAPSE+".csv")
df_enriched["enrichment"] = enriched_phrase
df_depleted = pd.read_csv("final_results_FL/final_table_all_tests_depleted_results_FL_"+LABEL+"_collapse_"+COLLAPSE+".csv")
...
# Remove leading and trailing spaces in the 'description' column
df['description'] = df['description'].str.strip()
# remove . at the end of the pfam_id
df["pfam_id"] = df["pfam_id"].str.split(".").str[0]
print (df)
df.to_csv("tables_to_export/FL_combined_results_"+LABEL+"_collapse_"+COLLAPSE+".csv", index=False)

#get pivot table ready to export
def remove_sums(enrichment):
    df_pivot = pd.read_csv("final_results_FL/FL_pivot_table_"+enrichment+"_results_"+COLLAPSE+"_"+LABEL+"_at_least_0_clades.csv",index_col=0)
    ...
    if (enrichment == "enriched"):
        output.to_csv("tables_to_export/FL_pivot_table_"+enriched_phrase+"_results_"+COLLAPSE+"_"+LABEL+".csv")
    else:
        output.to_csv("tables_to_export/FL_pivot_table_"+depleted_phrase+"_results_"+COLLAPSE+"_"+LABEL+".csv")
