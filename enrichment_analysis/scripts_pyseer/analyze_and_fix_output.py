...
for file in files:
	collapse = str(file.split("_")[3])
	clade = str(file.split("_")[6])
	sample = int(file.split("_")[8].split(".")[0])
...
reduced.to_csv("all_clade_sample_collapse_"+COLLAPSE+"_"+TYPE+"_output_"+LABEL+"_fdr_updated.csv", index=False)